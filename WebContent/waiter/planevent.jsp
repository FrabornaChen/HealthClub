...
<div id="blank"></div>
	...
	<h3>制定活动</h3>
	<br><br>
	<a href="javascript:add_activity()">添加场次</a>
	<br><br>
	<form action=<%=path + "planevent.action"%> method="post">
		<div>
			<label>时间：</label> 
			<input type="text" name="datehm" readOnly onClick="setDayHM(this);" id="text">
			<label>地点：</label> 
			<input type="text" name="address" id="text"> 
			<label>教练：</label> 
			<select name="coaches" id="text">
			<%
			...
			<label>场次：</label> 
			<input type="text" name="changci" readonly value="场次1" id="text"> 
		</div>
		<br> 
		<script type="text/javascript">
			...
			function add_activity() {
				num++;
				var new_member = "<div><label>时间：</label>"
						+ "\n<input type=\"text\" name=\"datehm\"  readOnly onClick=\"setDayHM(this);\" id=\"text\">"
						+ "\n<label>地点：</label>"
						+ "\n<input type=\"text\" name=\"address\" id=\"text\">"
						+ "\n<label>教练：</label>"
						+ "\n<select name=\"coaches\" id=\"text\">"
						+"<%for(int i=0;i<cid.length;i++){%>"
						+"<option><%=cid[i] %></option><%}%></select>"
						+ "\n<label>场次：</label>"
						+ "\n<input type=\"text\" name=\"changci\" value='场次"+num+"' id=\"text\"></div><br> ";
				document.getElementById("addactivity").insertAdjacentHTML(
						"beforeBegin", new_member);
			}
		</script>
		<input type="submit" id="addactivity" value="添加活动">
	</form>
	</div>
	...
	<script language="JavaScript">
		...
		document
				.write('<div id="divTiannetDate" style="'+s_tiannet_body+'" style="本日历选择控件由tiannet根据前人经验完善而成！" align="center">');
		document
		...
		for (var i = 0; i < 5; i++) {
			...
			for (var j = 0; j < 7; j++) {
				document
						.write('<td align="center" id="tdTiannetDay'
								+ n
								+ '" '
		...
		function spanYearCEvent() {
			hideElementsById(new Array("selTianYear", "tiannetMonthHead"),
					false);
			if (m_bolShowHour)
		...
		function spanMonthCEvent() {
			hideElementsById(new Array("selTianMonth", "tiannetYearHead"),
					false);
			if (m_bolShowHour)
				hideElementsById(new Array("tiannetHourHead"), false);
			if (m_bolShowMinute)
		...
		function tiannetWriteHead() {
			...
			if (m_bolShowHour)
				document.all.tiannetHourHead.innerText = " " + tiannetHour
						+ "时";
			if (m_bolShowMinute)
		...
		function tiannetSetDay(yy, mm) {
			...
			for (var i = 0; i < 37; i++) {
				...
				if (tiannetArrDay[i] != "") {
					...
					da.innerHTML = strDateFont1 + tiannetArrDay[i]
							+ strDateFont2;
					//如果是当前选择的天，则改变颜色 
		...
		function tiannetSetValue(bolHideControl) {
			...
			if (day < 10 && day.toString().length == 1)
				day = "0" + day;
			value = tiannetYear + tiannetDateSplit + mm + tiannetDateSplit
					+ day;
			if (m_bolShowHour) {
		...
		function tiannetPopCalendar() {
			//隐藏下拉框，显示相对应的head 
			hideElementsById(new Array("selTianYear", "selTianMonth",
					"selTianHour", "selTianMinute"), true);
			hideElementsById(new Array("tiannetYearHead", "tiannetMonthHead",
			...
			if (window.event.y + w > document.body.offsetHeight - 10)
				iY = document.body.scrollTop + document.body.offsetHeight - w
						- 5;
			else
